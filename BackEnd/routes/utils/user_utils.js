...
async function addPrivateRecipe(user_id,imageUrl,title,minutes,vegan,vegetarian,gluten,ingredients,instructions){
    await DButils.execQuery(`insert into PrivateRecipes values ('0','${user_id}','${imageUrl}','${title}','${minutes}','0',${vegan},${vegetarian},${gluten},'${ingredients}','${instructions}')`);
}
async function getPrivateRecipes(user_id){
    ...
    for (let i = 0 ; i<private_recipes.length; i++){
        var obj = {
            ...
            "image":private_recipes[i].imageUrl,
